...
int crystalCalib()
{
	...
	for(int event=0; event<ze2->Entries(); event++ ) //fill the data hists
	{
		...
		{	
			...
			if(ze2->reco.eta[1]>0)//positive endcap
			{
				...
				if( (ze2->reco.mz>75) && (ze2->reco.mz<105) )	ratioMapP[std::make_pair(ix,iy)]->Fill(ze2->reco.pt[1]/expectedPt);
			}
			else//negative endcap
			{
				histMapN[std::make_pair(ix,iy)]->Fill(ze2->reco.mz);
				if( (ze2->reco.mz>70) && (ze2->reco.mz<110) )	ratioMapN[std::make_pair(ix,iy)]->Fill(ze2->reco.pt[1]/expectedPt);
			}
		}
		ze2->GetNextEvent();
	}
	...
	for(int i=30;i<71;i++)
	{
		for(int j=30;j<71;j++)
		{
			...
			if( ring==0 )
			{
				std::cout<<"Ring 0 found."<<std::endl;
			}
			
			if( histMapP[std::make_pair(i,j)]->Integral() > 50 )
			{				
				sprintf(filename,"Efficiencies/CrystalCalib/mZix%diy%dzP.png",i,j);
				histMapP[std::make_pair(i,j)]->SetMarkerStyle(20);
				histMapP[std::make_pair(i,j)]->Fit(massFit,"Q","",80,110);
			...
			{
				sprintf(filename,"Efficiencies/CrystalCalib/mZix%diy%dzN.png",i,j);
				histMapN[std::make_pair(i,j)]->SetMarkerStyle(20);
				histMapN[std::make_pair(i,j)]->Fit(massFit,"Q","",80,110);
			...
			{
				ratioMapP[std::make_pair(i,j)]->Fit(ratioFit,"Q","",0.4,1.6);
				sprintf(filename,"Efficiencies/CrystalCalib/ratioIx%diy%dzP.png",i,j);
			...
			{
				ratioMapN[std::make_pair(i,j)]->Fit(ratioFit,"Q","",0.4,1.6);
				sprintf(filename,"Efficiencies/CrystalCalib/ratioIx%diy%dzN.png",i,j);
				sprintf(legend1, "Mean = %f +/- %f",ratioFit->GetParameter(1),ratioFit->GetParError(1));
				sprintf(legend2, "#chi^{2}/ndof = %f",ratioFit->GetChisquare()/20);
	...
	allMeans->Draw("E");
	c1->Print("Efficiencies/CrystalCalib/allMeans.png");
	c1->Clear();
	meansP->SetMarkerStyle(20);
	meansP->Draw("E");
	c1->Print("Efficiencies/CrystalCalib/MeansP.png");
	c1->Clear();
	meansN->SetMarkerStyle(20);
	meansN->Draw("E");
	c1->Print("Efficiencies/CrystalCalib/MeansN.png");
	c1->Clear();
	meansMapP->GetZaxis()->SetRangeUser(0,2);
	meansMapP->Draw("colz");
	c1->Print("Efficiencies/CrystalCalib/MeansMapP.png");
	c1->Clear();
	meansMapN->GetZaxis()->SetRangeUser(0,2);
	meansMapN->Draw("colz");
	c1->Print("Efficiencies/CrystalCalib/MeansMapN.png");
	c1->Clear();
	peaksMapP->GetZaxis()->SetRangeUser(80,110);
	peaksMapP->Draw("colz");
	c1->Print("Efficiencies/CrystalCalib/MassPeaksMapP.png");
	c1->Clear();
	peaksMapN->GetZaxis()->SetRangeUser(80,110);
	peaksMapN->Draw("colz");
	c1->Print("Efficiencies/CrystalCalib/MassPeaksMapN.png");
	c1->Clear();	
	
	std::cout<<"Now making the vs.Eta-hists."<<std::endl;
	for(int i=0;i<7;i++)
	{
		sprintf(filename,"Efficiencies/CrystalCalib/MassPeakPEtaRing%d.png",i);
		...
		c1->Print(filename);
		c1->Clear();
		sprintf(filename,"Efficiencies/CrystalCalib/MassPeakNEtaRing%dN.png",i);
		...
		c1->Print(filename);
		c1->Clear();
		sprintf(filename,"Efficiencies/CrystalCalib/MeanPEtaRing%d.png",i);
		...
		c1->Print(filename);
		c1->Clear();
		sprintf(filename,"Efficiencies/CrystalCalib/MeanNEtaRing%dN.png",i);
	...
	ringAveragePeakP->SetMarkerColor(kBlue+2);
	ringAveragePeakP->GetYaxis()->SetRangeUser(85,105);
	ringAveragePeakP->Draw("E");
	c1->Print("Efficiencies/CrystalCalib/AveragePeakVsEtaRingP.png");
	c1->Clear();
	ringAveragePeakN->SetMarkerStyle(20);
	ringAveragePeakN->SetMarkerColor(kBlue+2);
	ringAveragePeakN->GetYaxis()->SetRangeUser(85,105);
	ringAveragePeakN->Draw("E");
	c1->Print("Efficiencies/CrystalCalib/AveragePeakVsEtaRingN.png");
	c1->Clear();
	ringAverageMeanP->SetMarkerStyle(20);
	ringAverageMeanP->SetMarkerColor(kBlue+2);
	ringAverageMeanP->GetYaxis()->SetRangeUser(0.8,1.2);
	ringAverageMeanP->Draw("E");
	c1->Print("Efficiencies/CrystalCalib/AverageMeanVsEtaRingP.png");
	c1->Clear();
	ringAverageMeanN->SetMarkerStyle(20);
	ringAverageMeanN->SetMarkerColor(kBlue+2);
	ringAverageMeanN->GetYaxis()->SetRangeUser(0.8,1.2);
	ringAverageMeanN->Draw("E");
	c1->Print("Efficiencies/CrystalCalib/AverageMeanVsEtaRingN.png");
	c1->Clear();
		
	std::cout<<massCount<<" mass hists and "<<ratioCount<<" ratio hists made!"<<std::endl;
	c1->Close();
	return 0;
}
