...
int crystalCalibData02()
{
	...
	TH1D *allPeaks = new TH1D("allPeaks","Distribution of Peaks, all NT;mean",40,80,120);
	TH1D *peaksN = new TH1D("meansN","Distribution of Peaks, NT-;mean",40,80,120);
	TH1D *peaksP = new TH1D("meansP","Distribution of Peaks, NT+;mean",40,80,120);
	...
	for(int i=30;i<71;i++)
	{
		for(int j=30;j<71;j++)
		{
			...
			{				
				...
				histMapP[std::make_pair(i,j)]->SetMarkerSize(1.5);
				histMapP[std::make_pair(i,j)]->Draw("E");
				sprintf(filename,"Efficiencies/CrystalCalibData/mZix%diy%dzP.png",i,j);
				...
				c1->Clear();
				l1->Clear();*/
				allPeaks->Fill(massFit->GetParameter(1));
				peaksP->Fill(massFit->GetParameter(1));
			...
			if( histMapN[std::make_pair(i,j)]->Integral() > 50 )
			{
				...
				histMapN[std::make_pair(i,j)]->SetMarkerSize(1.5);
				histMapN[std::make_pair(i,j)]->Draw("E");
				sprintf(filename,"Efficiencies/CrystalCalibData/mZix%diy%dzN.png",i,j);
				sprintf(legend1, "Mean = %f +/- %f",massFit->GetParameter(1),massFit->GetParError(1));
				sprintf(legend2, "#chi^{2}/ndof = %f",massFit->GetChisquare()/20);
				...
				l1->Clear();*/
				allPeaks->Fill(massFit->GetParameter(1));
				peaksN->Fill(massFit->GetParameter(1));
			...
			if( ratioMapP[std::make_pair(i,j)]->Integral() > 50 )
			{
				...
				/*sprintf(filename,"Efficiencies/CrystalCalibData/ratioIx%diy%dzP.png",i,j);
			...
			if( ratioMapN[std::make_pair(i,j)]->Integral() > 50 )
			{
				...
				/*sprintf(filename,"Efficiencies/CrystalCalibData/ratioIx%diy%dzN.png",i,j);
				sprintf(legend1, "Mean = %f +/- %f",ratioFit->GetParameter(1),ratioFit->GetParError(1));
				sprintf(legend2, "#chi^{2}/ndof = %f",ratioFit->GetChisquare()/20);
	...
	//draw histograms:
	allPeaks->Fit(massFit,"QLM","",80,110);
	sprintf(legend1, "Mean = %f",massFit->GetParameter(1));
	sprintf(legend2, "#sigma = %f",massFit->GetParameter(2));
	allPeaks->SetMarkerStyle(20);
	l1->Draw();
	c1->Print("Efficiencies/CrystalCalibData/allPeaks.png");
	c1->Clear();
	l1->Clear();
	peaksN->Fit(massFit,"QLM","",80,110);
	sprintf(legend1, "Mean = %f",massFit->GetParameter(1));
	sprintf(legend2, "#sigma = %f",massFit->GetParameter(2));
	peaksN->SetMarkerStyle(20);
	l1->Draw();
	c1->Print("Efficiencies/CrystalCalibData/peaksN.png");
	c1->Clear();
	l1->Clear();
	peaksP->Fit(massFit,"QLM","",80,110);
	sprintf(legend1, "Mean = %f",massFit->GetParameter(1));
	sprintf(legend2, "#sigma = %f",massFit->GetParameter(2));
	peaksP->SetMarkerStyle(20);
	l1->Draw();
	c1->Print("Efficiencies/CrystalCalibData/peaksP.png");
	c1->Clear();
	l1->Clear();
