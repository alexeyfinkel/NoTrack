//This is going to be an attempt to create an actual efficiency-calculating script based on the smeared MC
...
int SmearTry03(double mean_low=0, double mean_high=0.1, double sigma_low=0, double sigma_high=0.1, int Nstep=10)
{
...
double doSmearing(double mean, double sigma, TFile* f1, TFile* f2, bool plot)
{
	...
	while(run)
	{
		...
		if( ((ze2->reco.eta[1])>2.61) && ((ze2->reco.eta[1])<3.0) && (ze2->reco.isSelected(1,NT)) && (ze2->reco.phi[1]>0) && (ze2->reco.phi[1]<0.182)
	...
	int nDat = datM->Integral();
	int nMC = newM->Integral();
	...
	if(plot)
	{
		...
		c2->Update();
		c2->Print("SmearingGaussOptimal_extraEta.png");
		c2->Close();
	}
