...
void fitStruct::doFit()
{
	...
	bkgdEvts = integral(bkgdFunct,82,102); //bkgdFunct->Integral(82,102);
}
...
int Efficiency02(double mean=0.059, double sigma=0.091)
{
	...
	//arrays to hold efficiency values:
	double effVsNV1[nvBins+2], effVsPt1[nvBins+2], effVsNV2[nvBins+2], effVsPt2[nvBins+2];
	...
	for(int i=0; i<=nvBins+1; i++)
	{
		...
		//std::cout<<"There are "<<sigEvtsBefore<<" signal events before the NT cut and "<<sigEvtAfter<<" events after in bin "<<i<<".\n"<<std::endl;
		//std::cout<<"The resulting efficiency is "<<(effVsNV1[i]*100)<<"%."<<std::endl;
	}
	...
	for(int j=0; j<=ptBins+1; j++)
	{
		...
		std::cout<<"\nBEFORE Cut, in Pt hist "<<j<<" there are "<<sigEvtsBefore1<<" events by peak fitting, and "<<sigEvtsBefore2<<" by bkgd subtraction"<<std::endl;
		std::cout<<"There are "<<before.datEvts<< "signal events and "<<before.bkgdEvts<<" background events in range"<<std::endl;
		std::cout<<"AFTER Cut, there are "<<sigEvtAfter1<<" events by peak fitting, and "<<sigEvtAfter2<<" by bkgd subtraction"<<std::endl;
		std::cout<<"There are "<<after.datEvts<< "signal events and "<<after.bkgdEvts<<" background events in range\n"<<std::endl;
	}
	...
	//comparison plots
	effVsNVert2->SetTitle("Efficiency vs. nVert Method Comparison");
	effVsNVert2->SetMarkerStyle(20);
	...
	effVsPtrans2->SetTitle("Efficiency vs. Pt Method Comparison");
	effVsPtrans2->SetMarkerStyle(20);
