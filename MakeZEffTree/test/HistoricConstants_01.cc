...
int reCalibrateData()
{
	...
	TH2D *histConstsN = new TH2D("histConstsN","Historical Constants I, NT-;i_{x};i_{y}",40,30,70,40,30,70);
	TH2D *histConstsP = new TH2D("histConstsp","Historical Constants I, NT+;i_{x};i_{y}",40,30,70,40,30,70);
    ...
    ifstream histConstsFile;
    //read in cumulative consts file
    histConstsFile.open("/afs/cern.ch/work/a/afinkel/public/NoTrack/IC_FILES/EcalIntercalibConstants_CRAFT2008_offline_69672.txt");
	if(!histConstsFile.is_open())
	{
		std::cout<<"Failed to open hist. cal. constants file. Existing"<<std::endl;
		return 1;
	}
    do
    {
        histConstsFile.getline(line, 1000)>>cix>>ciy>>iz>>calConst>>sigma;
        if(iz==0)continue;
        if(  (((cix-50.5)*(cix-50.5)+(ciy-50.5)*(ciy-50.5))  < 132.25) || (((cix-50.5)*(cix-50.5)+(ciy-50.5)*(ciy-50.5)) > 342.25)  ) continue;
        if(iz==-1) 
        {
            histConstsN->SetBinContent(cix-30,ciy-30,calConst);
        }
        else if(iz==1)
        {
            histConstsP->SetBinContent(cix-30,ciy-30,calConst);
        }
        else 
        {
            std::cout<<"Error reading in a cal. const. Exiting"<<std::endl;
    ...
    } while(!histConstsFile.eof());
    
    //and file to store absolute consts:
    ofstream absConstsFile;
    absConstsFile.open("Calibration/History/HistoricCumulativeConsts_01.txt");
		...
		for(int event=0; event<ze2->Entries(); event++ ) //fill the data hists
		{
			...
			{	
                ...
                {	
                    ...
                    if(ze2->reco.eta[1]>0) correction+=(constMapP[std::make_pair(hix,hiy)]->GetBinContent(iter-1))*(ze2->hitEnergyFractions->at(k))/decalConstsP->GetBinContent(hix-30,hiy-30)*histConstsP->GetBinContent(hix-30,hiy-30);//positive endcap 
                    else correction+=(constMapN[std::make_pair(hix,hiy)]->GetBinContent(iter-1))*(ze2->hitEnergyFractions->at(k))/decalConstsN->GetBinContent(hix-30,hiy-30)*histConstsN->GetBinContent(hix-30,hiy-30);//negative endcap
                }
    ...
    //PLOTS FOR DAYS!!
    absConstsN->GetZaxis()->SetRangeUser(0.5,1.5);
	//absConstsN->GetZaxis()->SetLabelSize(0.03);
	absConstsN->Draw("colz");
	c1->Print("Calibration/History/HistoricCumulativeConsts_01N.png");
	c1->Clear();
    absConstsP->GetZaxis()->SetRangeUser(0.5,1.5);
	//absConstsP->GetZaxis()->SetLabelSize(0.03);
	absConstsP->Draw("colz");
	c1->Print("Calibration/History/HistoricCumulativeConsts_01P.png");
	c1->Clear();
	absConsts1D->SetMarkerStyle(20);
	absConsts1D->SetMarkerSize(1.4);
	absConsts1D->Draw("P");
	c1->Print("Calibration/History/historicCumulativeConstsDist_01.png");
	c1->Clear();
    ...
    mZbefore->SetTitle("Z Peak after 2008 Calibration;M_{ee} (GeV)");
    ...
    voigtFit1->SetParameter(5,-10);
    voigtFit1->SetLineWidth(5);
    voigtFit1->SetLineColor(kRed+1);
    mZoriginal->SetMarkerStyle(20);
    mZoriginal->SetMarkerColor(kRed+1);
    mZoriginal->SetMarkerSize(1.4);
    mZoriginal->SetLineColor(kRed+1);
    mZoriginal->SetLineWidth(5);
    mZoriginal->Fit(voigtFit1,"QLMN","",70,110);
    mZoriginal->Draw("same p");
    voigtFit1->DrawClone("same l");
    sprintf(title,"Historic I: #mu = %.3g, #sigma = %.3g",(float)voigtFit1->GetParameter(1),(float)voigtFit1->GetParameter(2) );
    ...
    mZafter->Draw("same p");
    voigtFit1->DrawClone("same l");
    sprintf(title,"Recalibrated: #mu = %.3g, #sigma = %.3g",(float)voigtFit1->GetParameter(1),(float)voigtFit1->GetParameter(2) );
    l1->AddEntry(mZafter,title,"l");
    
    l1->Draw();
    c1->Print("Calibration/History/mZold-Hist01-New.png");
