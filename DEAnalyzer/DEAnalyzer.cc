   ...
   private:
      ...
      class CutLevels
	  {
		public:		
			...
			//TH1 *EveryMass, *ElecPt, *ElecEta, *PhotPt, *PhotEta, *PhotE1x5, *PhotE2x5, *PhotE3x3, *PhotE5x5, *PhotR9, *PhotSieie, *PhotHoEM, *PhotonIso;
	  } ;
...
DEAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    std::cout << "YO-YO, YO! It's me, your code!!!" << std::endl;
    ...
    for (unsigned int i=0; i<recoElectrons->size(); i++)
    {
    	...
    	for (unsigned int j=0; j<recoGammas->size(); j++)
    	{
    		...
    		dev.PhotPt = ecoGammas->at(j).p4().Pt();
    		dev.PhotEta = ecoGammas->at(j).p4().Eta();
